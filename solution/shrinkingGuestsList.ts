let guests02: string[] = ['Imran', 'Kashif', 'Farhan', 'Asif'];

console.log(
    `Hey [${guests02.join(
        ',',
    )}], we have more guests on borad so need to change the table.`,
);

let newGuestsList: string[] = ['Ali', 'Numan', 'Bilal'];
...
console.log(
    'We are unable to find a big table for you guyss, so just 2 guests are allowed for now.',
);
